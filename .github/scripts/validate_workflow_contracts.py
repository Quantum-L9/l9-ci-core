...
def _load_debt(root: Path, result: GateResult):
    ...
    if errors:
        for err in errors:
            result.add_violation(
                code="DEBT_SCHEMA_INVALID", message=schema_loader.format_error(err)
            )
        result.result = ResultStatus.error
        return set()
    active = set()
    ...
    for entry in (data or {}).get("debt", []):
        try:
            ...
            result.add_violation(
                code="DEBT_SCHEMA_INVALID",
                message=f"invalid created_at/expires_on: {entry.get('created_at')!r}/{entry.get('expires_on')!r}",
            )
            result.result = ResultStatus.error
            continue
        if expires < today:
...
def _validate_public_contract(root: Path, wf, rel: str, result: GateResult):
    ...
    iface_path = (
        root / "tests" / "fixtures" / "workflow-contracts" / "pr-pipeline-public-interface.json"
    )
    if not iface_path.exists():
        result.add_violation(
            code="PUBLIC_INTERFACE_FIXTURE_MISSING",
            message=(
                "pr-pipeline.yml exists but its frozen public-interface "
    ...
    if not has_wc:
        result.add_violation(
            code="PUBLIC_INTERFACE_BREAK",
            message=f"{rel}: must declare on.workflow_call to honor its public interface.",
            path=rel,
        )
        result.result = ResultStatus.failed
        return
    ...
    for name, spec in (iface.get("inputs") or {}).items():
        if name not in declared:
            result.add_violation(
                code="PUBLIC_INTERFACE_BREAK",
                message=f"{rel}: missing declared input {name!r}.",
                path=rel,
            )
            result.result = ResultStatus.failed
            continue
        ...
        if spec.get("type") and actual.get("type") != spec.get("type"):
            result.add_violation(
                code="PUBLIC_INTERFACE_BREAK",
                message=f"{rel}: input {name!r} type {actual.get('type')!r} != {spec.get('type')!r}.",
                path=rel,
            )
            result.result = ResultStatus.failed
        if bool(actual.get("required", False)) != bool(spec.get("required", False)):
            result.add_violation(
                code="PUBLIC_INTERFACE_BREAK",
                message=f"{rel}: input {name!r} required flag changed.",
                path=rel,
            )
            result.result = ResultStatus.failed
        if "default" in spec and actual.get("default") != spec.get("default"):
            result.add_violation(
                code="PUBLIC_INTERFACE_BREAK",
                message=f"{rel}: input {name!r} default {actual.get('default')!r} != {spec.get('default')!r}.",
                path=rel,
            )
            result.result = ResultStatus.failed
...
def run(root, output_json, fmt, quiet):
    ...
    for wf_path in workflow_files:
        ...
        if isinstance(triggers, dict) and "pull_request_target" in triggers:
            result.add_violation(
                code="PR_TARGET_FORBIDDEN",
                message=f"{wf_path.name}: pull_request_target is forbidden.",
                path=rel,
            )
            result.result = ResultStatus.failed
        ...
        for job_id, job in iter_jobs(wf):
            ...
            if is_bootstrap:
                if "permissions" not in job:
                    result.add_violation(
                        code="BOOTSTRAP_PERMISSIONS_MISSING",
                        message=f"{wf_path.name}: job={job_id} missing permissions block.",
                        path=rel,
                    )
                    result.result = ResultStatus.failed
                if "timeout-minutes" not in job:
                    result.add_violation(
                        code="BOOTSTRAP_TIMEOUT_MISSING",
                        message=f"{wf_path.name}: job={job_id} missing timeout-minutes.",
                        path=rel,
                    )
                    result.result = ResultStatus.failed
                for step_idx, step in iter_steps(job):
                    ...
                    if "upload-artifact" in uses_val or "upload-artifact" in step_name.lower():
                        ...
                        if "always()" not in if_cond:
                            result.add_violation(
                                code="BOOTSTRAP_EVIDENCE_NOT_ALWAYS",
                                message=f"{wf_path.name}: job={job_id} step={step_name!r} missing if: always().",
                                path=rel,
                            )
                            result.result = ResultStatus.failed
                        ...
                        if str(with_block.get("if-no-files-found", "")) != "error":
                            result.add_violation(
                                code="BOOTSTRAP_EVIDENCE_OPTIONAL",
                                message=f"{wf_path.name}: job={job_id} step={step_name!r} missing if-no-files-found: error.",
                                path=rel,
                            )
                            result.result = ResultStatus.failed
                    if run_block and len(run_block.strip()) > 40:
                        if _RE_UNSAFE_SHELL.search(run_block) and "status" not in run_block:
                            result.add_violation(
                                code="BOOTSTRAP_SHELL_UNSAFE",
                                message=f"{wf_path.name}: job={job_id} step={step_name!r} uses set +e without status capture.",
                                path=rel,
                            )
                            result.result = ResultStatus.failed
                    # Failure-swallowing is forbidden regardless of block size.
                    if run_block and _RE_SWALLOW.search(run_block):
                        if not _debt_matches(
                            active_debt, "FAILURE_SWALLOW_FORBIDDEN", rel, job_id, step_name
                        ):
                            result.add_violation(
                                code="FAILURE_SWALLOWED",
                                message=f"{wf_path.name}: job={job_id} step={step_name!r} swallows failure.",
                                path=rel,
                            )
                            result.result = ResultStatus.failed
                    ...
                    if (
                        run_block
                        and _is_nontrivial_run(run_block)
                        and not _RE_STRICT_SHELL.search(run_block)
                    ):
                        if not _debt_matches(
                            active_debt, "STRICT_SHELL_REQUIRED", rel, job_id, step_name
                        ):
                            result.add_violation(
                                code="STRICT_SHELL_REQUIRED",
                                message=f"{wf_path.name}: job={job_id} step={step_name!r} nontrivial run block missing 'set -euo pipefail'.",
                                path=rel,
                            )
                            result.result = ResultStatus.failed
