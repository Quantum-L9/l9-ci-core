...
_DOWNLOAD_CMD = re.compile(
    r"\bcurl\b|\bwget\b|Invoke-WebRequest|iwr\b|Invoke-RestMethod|irm\b|Start-BitsTransfer|--output\b|-OutFile\b"
)
...
_PWSH_GETFILEHASH = re.compile(
    r"Get-FileHash\b(?=[^\n]*?-Path\s+(?P<path>\S+))(?=[^\n]*?-Algorithm\s+SHA256\b)", re.IGNORECASE
)
...
_VAR_ASSIGN = re.compile(r'^\s*\$?([A-Za-z_][A-Za-z0-9_]*)\s*=\s*"?([^"\n]+?)"?\s*$', re.MULTILINE)
...
def run(root, registry_path, output_json, fmt, quiet):
    ...
    for wf_path in workflow_files:
        ...
        for jid, idx, name, run_block, line_no in iter_run_blocks(wf):
            ...
            if has_py:
                result.add_violation(
                    code="UNSUPPORTED_DOWNLOAD_PATTERN",
            ...
            if _PIPE_EXEC.search(run_block):
                result.add_violation(
                    code="STREAMED_EXECUTION_FORBIDDEN",
            ...
            if url_m is None:
                result.add_violation(
                    code="DOWNLOAD_URL_BINDING_MISSING",
            ...
            if sha_m is None:
                result.add_violation(
                    code="DOWNLOAD_DIGEST_BINDING_MISSING",
            ...
            if not has_verify:
                ...
                if has_pwsh and not posix_verify:
                    ...
                    result.add_violation(
                        code="DOWNLOAD_CHECKSUM_MISSING",
            ...
            # ... and must occur before the artifact is executed or unpacked.
                ...
                result.add_violation(
                    code="DOWNLOAD_VERIFIED_TOO_LATE",
