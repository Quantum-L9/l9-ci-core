...
def workspace_path(value: str) -> Path:
    ...
    try:
        ...
        raise ConsumerMetadataError("consumer metadata path must remain inside GITHUB_WORKSPACE") from error
    return path
...
def load_metadata(path: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise ConsumerMetadataError(f"consumer metadata is not valid JSON: {error}") from error
    if not isinstance(payload, dict):
        raise ConsumerMetadataError("consumer metadata must be an object")
    unknown = sorted(set(payload) - ALLOWED_KEYS)
    if unknown:
        raise ConsumerMetadataError(f"consumer metadata contains forbidden keys: {unknown}")
    ...
    if owner and (not isinstance(owner, str) or not OWNER_RE.fullmatch(owner)):
        raise ConsumerMetadataError("owner must be an org/team-style pointer such as Quantum-L9/platform")
    repo_class = payload.get("repo_class", "auto")
    if repo_class not in ALLOWED_REPO_CLASSES:
        raise ConsumerMetadataError(f"repo_class must be one of {sorted(ALLOWED_REPO_CLASSES)}")
    waiver_refs = payload.get("waiver_refs", [])
    if not isinstance(waiver_refs, list) or not all(isinstance(item, str) and item for item in waiver_refs):
    ...
    if len(waiver_refs) > 32 or len(set(waiver_refs)) != len(waiver_refs):
        raise ConsumerMetadataError("waiver_refs must contain at most 32 unique identifiers")
    payload["owner"] = owner
