...
def _working_tree_files(root: Path) -> list[str]:
    unstaged = _lines(
        ...
        "unable to resolve working-tree changes",
    )
    staged = _lines(
        _run_git(root, "diff", "--cached", "--name-only"),
        "unable to resolve staged changes",
    )
    untracked = _lines(
        _run_git(root, "ls-files", "--others", "--exclude-standard"),
        "unable to resolve untracked files",
    )
...
def _comparison_files(root: Path, base_ref: str, head_ref: str) -> list[str]:
    ...
    return _lines(
        _run_git(root, "diff", "--name-only", bases[0], head_ref),
        f"unable to diff {bases[0]} and {head_ref}",
    )
...
def resolve_changed_files(
    ...
    *,
    ...
    if explicit:
        files = tuple(sorted(dict.fromkeys(path for path in explicit if path)))
