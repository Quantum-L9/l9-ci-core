...
def test_existing_eight_preflight_gates_preserved():
    report = ev.evaluate({}, {}, None, None)
    assert [g["id"] for g in report["gates"]] == [1, 2, 3, 4, 5, 6, 7, 8]
...
def test_dry_run_adapter_is_default_and_never_pushes(git_repo: Path):
    ad = get_adapter(dry_run=True, enable_gh=True)  # even with enable_gh, dry-run wins
