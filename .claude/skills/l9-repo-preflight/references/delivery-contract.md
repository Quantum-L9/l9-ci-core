...
# Delivery, Accounting & Reporting Contract (v2.1.0)

Separated, independently testable concerns layered on the fail-open loop. All
remote effects go through a **replaceable GitHub adapter** (`DryRunAdapter` default,
`GhCliAdapter` when `--enable-gh` and `gh` present), are **idempotent**, and return
**receipts**; nothing claims a remote action it did not perform.
