...
- **`Repo.local.mk`** is tracked and repository-owned. It owns Core-specific
  leaves such as `change-policy`, `agent-check`, `reconcile`, and release
  assurance. It may add native targets but may not override a generated,
  universal, or reserved Governance target.
...
- `make make-render`: render `Repo.mk` from Core's default capability plan.
- `make make-check`: verify byte-for-byte `Repo.mk` freshness and reject a
  protected-target override in `Repo.local.mk`.
