...
The root `Makefile` is a stable generated bootstrap. It owns only operator
help and Governance delegation. Compiler V2 separates the standard adapter
from repository-native leaves:

- **`Repo.mk`** is generated by `tools/l9_make/` from Core's closed capability
  plan. It owns the six standard `repo-*` bindings and delegates them to the
  existing Core repository runtime.
- **`Repo.local.mk`** is tracked and repository-owned. It owns Core-specific
  leaves such as `change-policy`, `agent-check`, `status`, and release
  assurance. It may add native targets but may not override a generated target
  or a reserved Governance target.
...
neither defines a `pr` target or a `push` target. The compiler does not inspect
the repository, infer languages, invoke providers, or own SDK capability
semantics. An SDK-fed capability-plan envelope requires a separately approved
producer contract before it can replace the Core-default plan.
